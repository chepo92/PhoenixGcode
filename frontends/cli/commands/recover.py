...
class RecoverCommand(CLICommand):
    ...
    def register(self, app: typer.Typer) -> None:
        ...
        def recover(
            file: Path = typer.Argument(..., help="Archivo G-code original interrrumpido."),
            z: Optional[float] = typer.Option(None, "--z", help="Altura Z medida (mm)."),
            candidate: int = typer.Option(0, "--candidate", help="Índice del candidato a utilizar."),
            ...
            measured_z = z if z is not None else CLIPrompts.ask_measured_z()
            output_path = output or file.parent / f"{file.stem}_recovered{file.suffix}"
            ...
            try:
                ...
                plan_dto, final_out_str = CLIPrompts.prompt_edit_menu(plan_dto, str(output_path))
